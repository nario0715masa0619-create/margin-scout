...
def test_user_a_cannot_see_user_b_jobs(client, auth_headers_a, auth_headers_b):
    client.post("/api/v1/research-jobs/", json={"title": "Job B", "conditions": {"keywords": ["Nintendo"], "sources": ["mercari"], "days_back": 90, "min_sales": 2}}, headers=auth_headers_b)
...
def test_user_a_cannot_access_user_b_job_by_id(client, auth_headers_a, auth_headers_b):
    create_res = client.post("/api/v1/research-jobs/", json={"conditions": {"keywords": ["Nintendo"], "sources": ["mercari"], "days_back": 90, "min_sales": 2}}, headers=auth_headers_b)
